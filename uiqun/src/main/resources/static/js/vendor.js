        ...
        // if(checkCurrentUserInfoVar==null||checkCurrentUserInfoVar.uid!=$(obj).attr("value")) {
            $.post("/user/queryUserInfo", {"userId": $(obj).attr("value")}, function (result) {
                if (result != null) {
                    ...
                    $("#currentUserInfo tbody>tr:first-child>td:nth-of-type(2)").html(result.contact==null?"":result.contact);
                    $("#currentUserInfo tbody>tr:first-child>td:nth-of-type(4)").html(result.tel==null?"":result.tel);
                    $("#currentUserInfo tbody>tr:nth-of-type(2)>td:nth-of-type(2)").html(result.qq==null?"":result.qq);
                    $("#currentUserInfo tbody>tr:nth-of-type(2)>td:nth-of-type(4)").html(result.wechat==null?"":result.wechat);
